...
public class BoardAdviceDao {
	...
	public BoardAdvice getBoardAdvice(int bid) {
		...
		String sql = "SELECT * FROM boardadvice" + "	WHERE bid=?";
		...
		try {
			...
			while (rs.next()) {
				boardAd = new BoardAdvice(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4),
						LocalDate.parse(rs.getString(5).split(" ")[0]), rs.getInt(6), rs.getInt(7), rs.getInt(8));
			}
			rs.close();
			pstmt.close();
			conn.close();
	...
	public List<BoardAdvice> getBoardAdviceList(String field, String query, int num, int offset) {
		Connection conn = getConnection();
		String sql = "SELECT * FROM boardadvice" + " where " + field + " like ?" + "	ORDER BY bid DESC "
				+ "	LIMIT ? OFFSET ?";
	...
	public void increaseCount(String field, int bid) {
	    Connection conn = getConnection();
	    String sql = "UPDATE boardadvice SET " + field + " = " + field + " + 1 WHERE bid=?";
	    try {
	        PreparedStatement pstmt = conn.prepareStatement(sql);
	        pstmt.setInt(1, bid);
	...
	public int getBoardAdviceCount(String field, String query) {
		Connection conn = getConnection();
		query = "%" + query + "%";
		String sql = "SELECT COUNT(bid) FROM boardadvice" + "  JOIN users ON boardadvice.uid=users.uid" + "  WHERE "
				+ field + " LIKE ?";
		int count = 0;
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);
